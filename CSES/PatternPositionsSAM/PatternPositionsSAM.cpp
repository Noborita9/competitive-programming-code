...
struct state {int len, fpos ,link;map<char,int> next;}; //clear next!!
const int N = 1e5 + 10;
...
void sa_extend(char c){
	int k=sz++,p;
	st[k].len=st[last].len+1;st[k].fpos=st[k].len-1;
...
void solve()
{
    ...
    L(qi,0,q){
        ...
        int v = 0;
        int i = 0;
        while(i < n && st[v].next.count(p[i])) {
            v = st[v].next[p[i]];
            i ++;
        }
        if (i == SZ(p)){
            cout << st[v].fpos - SZ(p) + 2 << "\n";
        } else {
            cout << "-1\n";
        }
    }
